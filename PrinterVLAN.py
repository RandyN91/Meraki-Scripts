            ...
            if 'MS' in devices['model']:
                ...
                try:
                    for clients in clients:
                        print(clients)
                        if  'dell' or 'Dell' in clients['description'] or \
                            'dell' or 'Dell' in clients['mdnsName'] or \
                            'dell' or 'Dell' in clients['dhcpHostname'] \
                            or str(clients['mac']).startswith('d8:9e:f3'):
                         print("Device Name : "+networks['name'])
                         print("Device MAC : "+clients['mac'])
                         print("Device Desc : "+clients['description'])
                         print("Device VLAN : "+clients['vlan'])
                         print("Device IP : "+clients['ip'])

                except:
                    print(TypeError + "No Data from " + networks['name'])
